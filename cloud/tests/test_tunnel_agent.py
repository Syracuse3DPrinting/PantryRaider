...
def test_upsert_peer_adds_and_updates():
    ...
    assert len(peers) == 1
    # Updating the same key replaces the entry rather than duplicating it.
    peers = agent.upsert_peer(peers, "K1", "10.99.0.9", "a.example")
    assert len(peers) == 1
...
def test_apply_add_shells_out_without_a_shell(tmp_path, monkeypatch):
    ...
    # The Caddy include now carries the kitchen.
