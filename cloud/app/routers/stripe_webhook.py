...
def _plan_for_price(price_id: str) -> str:
    """Map a live Stripe price id to its plan. Cloud Basic has one yearly
    price; Premium has a monthly and a yearly one. The old
    CLOUD_STRIPE_PRICE_STARTER id is kept working as a premium alias
    (deprecated); the price_to_plan dict covers any future tiers."""
    if price_id:
        if price_id == settings.stripe_price_basic_year:
            return "basic"
        if price_id in (settings.stripe_price_premium_month,
