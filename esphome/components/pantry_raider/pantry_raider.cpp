...
void PantryRaiderHub::step_sweep_() {
  if (this->sweep_phase_ == SWEEP_HEALTH) {
    ...
    if (this->health_probe_(htonl(hit.ip), hit.port, mode)) {
      ...
      if (mode == "server" || mode == "pi_hosted")
        rank = 0;
      else if (mode == "pi_remote")
        rank = 2;  // a satellite cannot pair; only used if nothing else answers
      SweepCandidate c;
  ...
  if (launched > 0) {
    struct timeval tv{};
    tv.tv_sec = SWEEP_CONNECT_MS / 1000;
    tv.tv_usec = (SWEEP_CONNECT_MS % 1000) * 1000;
    ::select(maxfd + 1, nullptr, &wset, nullptr, &tv);
