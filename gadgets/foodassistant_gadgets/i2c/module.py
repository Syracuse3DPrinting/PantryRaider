    ...
    def _render(self, now: float) -> None:
        ...
        for dev_id, dev in devices.items():
            ...
            if timer is not None:
                ...
                # way a number on a screen is not.
...
def _timer_view(outputs: dict, outputs_at: float, now: float) -> dict | None:
    ...
    if not isinstance(timer, dict):
        return None
    if not timer.get("running") or timer.get("ringing"):
        # Nothing running, or it is ringing: a ringing timer is an alarm and
        # the alarm surface owns the keys, so the bar gets out of its way.
        return None
