...
def _display_off_logo_due(display_blanked, enabled, deck_idle_blanked,
                          overlay_active, waking,
                          screensaver_active=False) -> bool:
    """Whether the deck should show the display-off logo face right now.

    Pure decision for the poll loop (FoodAssistant-zttc, FoodAssistant-qh8p):
    the kiosk screen is asleep by EITHER path (the bridge blanked the physical
    panel, OR the kiosk screensaver overlay is covering a still-lit panel), the
    setting is on, the deck has not blanked on its own timeout (a dark deck
    stays dark), no full-deck camera overlay owns the keys, and this very poll
    is not already waking everything up.
    """
    screen_asleep = display_blanked or screensaver_active
    return bool(screen_asleep and enabled and not deck_idle_blanked
...
class Controller:
    ...
    def _apply_display_state(self, display_blanked: bool, waking: bool,
                             screensaver_active: bool = False) -> None:
        ...
        kiosk display is blanked AND whether the kiosk screensaver overlay is
        showing (a soft sleep on a still-lit panel, FoodAssistant-qh8p), and
        ``waking`` is True when this same poll found activity on another surface
        (which is about to relight the display anyway)."""
    ...
    async def _poll_shared_activity(self) -> None:
        ...
        show the raccoon while the screen sleeps by EITHER the physical panel
        blank or the kiosk screensaver overlay (FoodAssistant-qh8p), and
        return to the page the moment it wakes (FoodAssistant-zttc)."""
        url = getattr(self.config, "host_bridge_url", "")
        if not url:
            return
        try:
        ...
        self._apply_display_state(
            bool(data.get("display_blanked")), wake,
            bool(data.get("screensaver_active")),
        )
