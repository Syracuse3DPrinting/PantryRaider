...
# The per-device threshold fields stored for the alarms follow-up
# (FoodAssistant-5c61). Stored and exposed through the API and Settings now,
# not yet acted on: no alert fires from them until that bead lands.
...
def ingest(payload: dict, now: float | None = None, *,
    ...
    with _lock:
        ...
        for entry in devices_in or []:
            ...
            # existing changes) or "hygrometer" (FoodAssistant-q97i).
...
def get_state(now: float | None = None) -> dict:
    ...
    for dev in hygro_conf:
        ...
        hygrometers.append({
            ...
            # Stored for the alarms follow-up (FoodAssistant-5c61); nothing
            # fires from these yet, but the UI can already edit them.
