...
PROTOCOLS = ("inkbird", "thermopro", "combustion", "bluedot")

_lock = threading.Lock()
# In-process view of the state file: devices/discovered/alerts, plus the file
# mtime the view corresponds to (None = never seen).
_state: dict = {"devices": {}, "discovered": {}, "alerts": {}}
    ...
    for probe in entry.get("probes") or []:
        ...
        if temp is not None:
            ...
            if not (-100.0 <= temp <= 600.0):
                temp = None
        probes.append({"index": index, "temp_c": temp})
    if not probes:
        return None
...
def ingest(payload: dict, now: float | None = None) -> dict:
    """Store a reader push (readings + discovered devices) and fire alerts."""
...
def reset() -> None:
    ...
    global _state, _mtime
    with _lock:
        _state = {"devices": {}, "discovered": {}, "alerts": {}}
        _mtime = None
        try:
