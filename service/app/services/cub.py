...
def decide_view(timers: list[dict], probes: list[dict], merged: dict) -> str:
    """What the Cub should show right now. Pure.

    Priority (docs/design/bandit-cub.md): "timers" when any timer is running
    or ringing and the takeover toggle is on; else "probe" when a probe has a
    target set and takeover is on; else the configured default view.
    ``timers`` is the summary's timers block (running and just-expired, i.e.
    ringing, entries alike take the display).
    """
