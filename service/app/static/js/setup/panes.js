...
function _gadgetsRender(d) {
  ...
  // Configured thermometers: name, live probes, battery, remove.
  ...
  if (devEl) {
    ...
    if (!devices.length) {
      ...
      devEl.innerHTML = devices.map(dev => {
        const probes = (dev.probes || [])
          .map(p => _gadgetsFmtTemp(p.temp_c, d.unit)).join(' · ');
        const bits = [];
        if (dev.protocol === 'home_assistant') bits.push('via Home Assistant');
        if (probes) bits.push(dev.stale ? 'last seen: ' + probes : probes);
        else bits.push(dev.stale ? 'not reporting' : 'no probe readings yet');
        if (dev.battery != null) bits.push(`battery ${dev.battery}%`);
        return `<div class="d-flex align-items-center gap-2 small mb-1">
          ...
          <span><strong>${_gEsc(dev.name || dev.id)}</strong>
            <span class="text-secondary">${_gEsc(bits.join(' · '))}</span></span>
