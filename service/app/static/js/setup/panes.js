...
async function loadBluetoothPrintStatus() {
  ...
  try {
    ...
    if (btn) {
      // While installing the button is disabled and shows its spinner; once
      // setup settles (ready/failed/not set up) restore the label so it does
      // not spin forever after a successful run.
      btn.disabled = status === 'installing';
