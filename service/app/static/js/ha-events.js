...
(function () {
  ...
  // the scan screen, and a button that takes seconds to move the screen feels
  // broken. So events poll fast on their own cheap channel (/events/poll is an
  // in-memory read of the event ring) instead of riding the slower consolidated
  // status the heavier surfaces share. Backs off when the server blips, and
  // skips entirely while the tab is hidden.
  var POLL_MS = 750;
  var BACKOFF_MAX_MS = 15000;
  var curDelay = POLL_MS;
  ...
  function poll() {
    ...
    fetch('events/poll?since=' + lastId, { cache: 'no-store' })
      ...
      .then(function (d) {
        // A good answer snaps back to the fast cadence; a blip backs off so a
        // down main server (a satellite whose forward failed) is not hammered.
        if (d) { curDelay = POLL_MS; applyEvents(d); }
        else { curDelay = Math.min(curDelay * 2, BACKOFF_MAX_MS); }
      })
      .catch(function () { curDelay = Math.min(curDelay * 2, BACKOFF_MAX_MS); })
      .finally(function () { setTimeout(poll, curDelay); });
  }

  // Events get their own fast, dedicated poll (see POLL_MS above): a physical
  // control's on-screen response has to feel immediate, and the consolidated
  // status poll the other surfaces share runs far slower (a 2s floor, often
  // 4s), which made a NeoKey press take seconds to move the screen. Read the
  // current last id first so a page opened long after an event does not replay
  // a backlog, then poll.
