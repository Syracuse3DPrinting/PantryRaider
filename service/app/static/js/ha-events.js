...
(function () {
  ...
  // confirmations (FoodAssistant-rdlo) ignore this: they are local feedback for
  // a Stream Deck press, not Home Assistant traffic, so they always show. That
  // is why the script no longer bails out when HA events are turned off.
  ...
  // deck confirmation always shows, whether HA events are on or off.
  function shouldShow(ev) {
    return !!ev && (enabled || ev.type === 'confirm');
  }
  ...
  function handle(ev) {
    ...
    // A confirmation renders exactly like a notification toast; it is just
    // gated differently (always shown) and coloured by its success level.
    if (ev.type === 'notification' || ev.type === 'confirm') showToast(ev);
