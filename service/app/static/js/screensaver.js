...
(function () {
  ...
  var EXTERNAL_GRACE_MS = (EXTERNAL_ACTIVITY_WINDOW_SECS + 2) * 1000;
  var ssSeedExternal = false;
  var ssExternalGraceUntil = 0;

  // externalWakeDue narrowed for a manually-opened saver. During the grace
  // window, mere freshness must NOT dismiss (that is the stale stamp from the
  // tap that opened it); only a strict advance past the open baseline (activity
  // that happened after the open) counts. Outside the grace window it is
  // exactly externalWakeDue. Pure, tested through node.
      ...
      fetch('setup/kiosk/activity', { cache: 'no-store' })
        ...
        .then(function (d) {
          ...
          var res = screensaverExternalWake(
            la, Date.now() / 1000, seenExternal,
            Date.now() < ssExternalGraceUntil);
  ...
  window.__screensaverTest = function (opts) {
    ...
    // Manual open: seed the external-wake baseline on the next poll and give it
    // a grace window, so the still-fresh activity from the tap that opened the
    // saver does not immediately dismiss it (FoodAssistant-qh8p follow-up).
    ssSeedExternal = true;
    ssExternalGraceUntil = Date.now() + EXTERNAL_GRACE_MS;
    show();
  };
})();
