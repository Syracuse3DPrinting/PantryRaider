...
(function () {
  ...
  // Draw a winged toaster in unit coordinates (the caller scales it): a chrome
  // body faked with flat highlight/shadow bands instead of a per-frame gradient,
  // two slots with slices of toast rising out, a side lever, and a wing that
  // flaps through the given angle. pink swaps the chrome for brand pink, the
  // Pantry Raider wink on the After Dark tribute. Every colour is a literal, so
  // nothing is allocated per frame.
  function drawToaster(ctx, wing, pink) {
    var body = pink ? '#f2006e' : '#c7ccd4';
    var hi = pink ? '#ff5aa0' : '#eef1f5';
    var lo = pink ? '#a80050' : '#8f949c';
    // Two slices of toast rising from the slots.
    ctx.fillStyle = '#d9a25a';
    rrect(ctx, -0.62, -1.02, 0.5, 0.62, 0.12); ctx.fill();
    rrect(ctx, 0.12, -1.02, 0.5, 0.62, 0.12); ctx.fill();
    ctx.fillStyle = '#b9793a';
    rrect(ctx, -0.62, -1.02, 0.5, 0.14, 0.1); ctx.fill();
    rrect(ctx, 0.12, -1.02, 0.5, 0.14, 0.1); ctx.fill();
    // Chrome body with fake shading bands.
    ctx.fillStyle = body;
    rrect(ctx, -1, -0.55, 2, 1.25, 0.28); ctx.fill();
    ctx.fillStyle = hi;
    rrect(ctx, -0.9, -0.48, 1.8, 0.3, 0.16); ctx.fill();
    ctx.fillStyle = lo;
    rrect(ctx, -0.9, 0.42, 1.8, 0.22, 0.14); ctx.fill();
    // Slots on the top surface.
    ctx.fillStyle = '#2a2d33';
    rrect(ctx, -0.66, -0.5, 0.56, 0.16, 0.08); ctx.fill();
    rrect(ctx, 0.1, -0.5, 0.56, 0.16, 0.08); ctx.fill();
    // Lever.
    ctx.fillStyle = lo;
    rrect(ctx, 0.92, -0.1, 0.16, 0.34, 0.07); ctx.fill();
    // Flapping wing on the back-left shoulder.
    ctx.save();
    ctx.translate(-0.85, -0.15);
    ctx.rotate(wing);
    ctx.fillStyle = pink ? '#ffd1e6' : '#f7f9fc';
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.quadraticCurveTo(-1.15, -0.5, -1.5, 0.05);
    ctx.quadraticCurveTo(-1.05, 0.2, 0, 0.28);
    ctx.closePath();
    ctx.fill();
    ctx.strokeStyle = pink ? '#e58bb5' : '#c9d2dc';
    ctx.lineWidth = 0.05;
    ctx.beginPath();
    ctx.moveTo(-0.2, 0.04); ctx.lineTo(-0.95, -0.16);
    ctx.moveTo(-0.2, 0.14); ctx.lineTo(-0.95, 0.02);
    ctx.stroke();
    ctx.restore();
  }
  ...
  function startCanvasMode(kind) {
    ...
    var corners = ['right:4vmin;bottom:4vmin;', 'left:4vmin;bottom:4vmin;',
                   'left:4vmin;top:4vmin;', 'right:4vmin;top:4vmin;'];
    var cornerIdx = 0;
    var clock = document.createElement('div');
    clock.className = 'ss-time';
    function placeClock() {
  ...
  if (kiosk || ALL_CLIENTS) {
    ...
    var pollExternalActivity = function () {
      fetch('setup/kiosk/activity', { cache: 'no-store' })
        ...
        .then(function (d) {
          if (!d) return;
          var res = externalWakeDue(
            typeof d.last_activity === 'number' ? d.last_activity : 0,
            Date.now() / 1000, seenExternal);
