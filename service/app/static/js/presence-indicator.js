...
//   - the indicator mounts only while the host bridge reports the sensor as
//     actually readable (presence_readable), so a kiosk without the sensor,
//     or one whose sensor stops answering, shows nothing at all.
//
...
(function () {
  ...
  function apply(data) {
    // presence_readable is the mount gate: only a bridge that can actually
    // sample the sensor answers true. Anything else (off a Pi, an older
    // bridge without the field, a sensor that stopped answering, an error
    // reply) hides the indicator entirely. Wake on presence "off" hides it
    // too: the bridge's presence loop idles in that mode, so the detected
    // value freezes at its last reading, and a diagnostic that shows a stale
    // value as live is worse than none.
    if (!data || data.ok !== true || data.presence_readable !== true
        || data.wake_on_presence === 'off') {
      unmount();
      return false;
    }
    mount();
