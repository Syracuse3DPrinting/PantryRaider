...
APTCONF
  ...
  # docker.io does not always pull the compose v2 plugin on its own; make sure the
  # `docker compose` subcommand exists, since the stack's compose files need it.
  if command -v docker >/dev/null 2>&1 && ! docker compose version >/dev/null 2>&1; then
    apt_install docker-compose-v2 || apt_install docker-compose-plugin \
      || warn "Docker installed but the Compose v2 plugin is missing; the stack may not start"
  fi
