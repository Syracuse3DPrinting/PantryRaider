...
labels, and stored min/max alert ranges. `gadget_ha_hygrometers` lists Home
Assistant temperature + humidity entity pairs read as hygrometers (under the
same `gadget_ha_enabled` toggle). All device-local, like the thermometer
fields.
