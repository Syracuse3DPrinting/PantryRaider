...
alerts when a probe reaches its target. The Bluetooth radio belongs to the
host, not the app container, so a small host-side reader service (installed
on request by `foodassistant-gadgets-setup`, or with `ENABLE_GADGETS=true` at
provision time) reads the thermometers and reports to the app.

`gadgets_enabled` is the master switch, off by default; adding your first
thermometer from the Timers page turns it on. `gadget_devices` holds the added
thermometers and their per-probe targets, managed entirely from the Timers
page. Both are device-local: a thermometer lives near one device's radio.
