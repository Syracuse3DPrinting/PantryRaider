...
def test_the_timer_bar_drains_as_the_soonest_timer_comes_due():
    ...
    assert timer_bar_levels(300, 600) == [1.0, 1.0, 0.0, 0.0]   # halfway
    ...
    assert timer_bar_levels(75, 600)[0] == pytest.approx(0.5)
...
def test_only_the_head_of_the_bar_breathes():
    ...
    assert trough[0] == peak[0]        # the settled part of the bar is steady
    assert trough[1] != peak[1]        # the head breathes
    assert trough[1] < peak[1]
    assert trough[2] == peak[2] == (0, 0, 0)    # spent keys stay dark
...
def test_the_timer_bar_defaults_to_brand_pink():
    ...
    assert timer_colors(600, 600, color=(0, 255, 0), brightness=100)[0] == (0, 255, 0)
...
def test_the_keys_go_back_to_the_modes_when_nothing_is_running():
    ...
    # A ringing timer is an alarm and the alarm surface owns the keys.
    assert _timer_view({"timer": {"running": 1, "ringing": True,
                                  "soonest_remaining": 0}},
                       1000.0, 1000.0) is None
