...
def test_screensaver_js_seeds_external_baseline_on_manual_open():
    ...
    # __screensaverTest) seeds the external-wake baseline and a grace window so
    # the still-fresh activity from the tap that opened it cannot dismiss it.
    assert "ssSeedExternal = true;" in js
    assert "ssExternalGraceUntil = Date.now() + EXTERNAL_GRACE_MS;" in js
    # The poll narrows dismissal through the grace-aware helper.
