...
# -- display-off logo on a soft (screensaver) sleep (FoodAssistant-qh8p) -------


def test_display_off_logo_due_shows_on_screensaver_or_physical_blank():
    from foodassistant_streamdeck.controller import _display_off_logo_due
    # Physical panel blank (existing path): logo shows.
    assert _display_off_logo_due(True, True, False, False, False) is True
    # Soft sleep: the kiosk screensaver overlay is up on a still-lit panel, the
    # deck is awake, the logo is enabled -> the logo shows.
    assert _display_off_logo_due(False, True, False, False, False,
                                 screensaver_active=True) is True
    # Neither path asleep -> no logo.
    assert _display_off_logo_due(False, True, False, False, False,
                                 screensaver_active=False) is False


def test_display_off_logo_due_screensaver_respects_the_other_gates():
    from foodassistant_streamdeck.controller import _display_off_logo_due
    # logo_when_display_off is off -> never, even on a soft sleep.
    assert _display_off_logo_due(False, False, False, False, False,
                                 screensaver_active=True) is False
    # The deck has slept on its own idle timeout -> a dark deck stays dark.
    assert _display_off_logo_due(False, True, True, False, False,
                                 screensaver_active=True) is False
    # A full-deck camera overlay owns the keys -> no logo.
    assert _display_off_logo_due(False, True, False, True, False,
                                 screensaver_active=True) is False
    # This very poll is waking everything -> no logo (a wake exits the face).
    assert _display_off_logo_due(False, True, False, False, True,
                                 screensaver_active=True) is False


def test_poll_shared_activity_raises_logo_on_screensaver(monkeypatch):
    import foodassistant_streamdeck.controller as controller_mod

    class _Resp:
        def json(self):
            # Panel still lit (not blanked), but the kiosk screensaver is up and
            # there is no fresh activity, so the deck should raise its logo.
            return {"last_activity": 0, "display_blanked": False,
